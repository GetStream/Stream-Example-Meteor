...
Router.route('/flat', {
  ...
  waitOn: function() {
    ...
    // They are named Stream.feed.feedGroupName
    // where feedGroupName is the name of a feed group
    return Meteor.subscribe('Stream.feed.flat');
  },
...
Router.route('/profile/:_user', {
  ...
  onBeforeAction: function() {
    ...
    if(user) {
      ...
      // i.e. limit)
      this.subscribe('Stream.feed.user', 20, user._id).wait();
    }
...
Router.route('/aggregated_feed', {
  ...
  waitOn: function() {
    return Meteor.subscribe('Stream.feed.aggregated');
  },
